...
const WalletCheckoutUtil = {
    ...
    try {
      ...
      for (const payment of acceptedPayments) {
        ...
        if (payment.contractInteraction) {
          ...
          if (!payment.contractInteraction.data || !Array.isArray(payment.contractInteraction.data)) {
      ...
      if (error instanceof z.ZodError) {
        ...
        throw createCheckoutError(CheckoutErrorCode.INVALID_CHECKOUT_REQUEST, `Validation failed: ${errorDetails}`);
      }
      throw error;
    }
  },
  ...
  async getFeasiblePayments(checkoutRequest: CheckoutRequest): Promise<{
    ...
    try {
      ...
      const { feasibleContractPayments, isUserHaveAtleastOneMatchingAssets: validContractPayments } =
      ...
      const feasiblePayments: DetailedPaymentOption[] = [...feasibleDirectPayments, ...feasibleContractPayments];
