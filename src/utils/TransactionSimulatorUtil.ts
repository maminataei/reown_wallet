...
const TransactionSimulatorUtil = {
  ...
  canTransactionSucceed: async (
    ...
    const client = createPublicClient({
      transport: http(`https://rpc.walletconnect.org/v1?chainId=eip155:${chainId}&projectId=${projectId}`),
    });
    ...
    try {
      ...
      const results = await Promise.all(
        calls.map(async (call) => {
          try {
            ...
            console.error(`Transaction simulation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return false;
          }
        })
      );
      ...
      console.error(`Error in transaction simulation: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  },
};
