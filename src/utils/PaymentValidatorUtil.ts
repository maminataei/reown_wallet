...
import { EIP155_CHAINS } from '../data/etherData';
...
export class PaymentValidationUtils {
  ...
  private static getAssetDetails(asset: string) {
    ...
    if (chainAssetParts.length !== 2) throw new Error('Invalid asset value, must be in CAIP-19 format');
   ...
   */
  private static async getNativeAssetDetails(chainId: number, account: `0x${string}`): Promise<TokenDetails> {
   ...
   */
  private static async getErc20TokenDetails(tokenAddress: Hex, chainId: number, account: Hex): Promise<TokenDetails> {
    const publicClient = createPublicClient({
      chain: getChainById(chainId),
      transport: http(EIP155_CHAINS[`eip155:${chainId}`].rpc),
    });
  ...
  private static async simulateAndGetNativeTokenPaymentDetails(
    ...
    const tokenDetails = canTransactionSucceed
      ? await PaymentValidationUtils.getNativeAssetDetails(Number(chainId), senderAccount as `0x${string}`)
  ...
  private static async getDetailedDirectPaymentOption(
    ...
    try {
      ...
      // Parse asset details
      const { chainId, assetAddress, chainNamespace, assetNamespace } = PaymentValidationUtils.getAssetDetails(
        payment.asset
      );
  ...
  private static async getDetailedContractPaymentOption(
    ...
    try {
      ...
      // Parse asset details
      const { chainId, assetAddress, chainNamespace, assetNamespace } = PaymentValidationUtils.getAssetDetails(asset);
      ...
      if (assetNamespace === 'erc20') {
        ...
        tokenDetails = await PaymentValidationUtils.getNativeAssetDetails(Number(chainId), account as `0x${string}`);
      }
  ...
  static async findFeasibleDirectPayments(directPayments: PaymentOption[]): Promise<{
    ...
    const results = await Promise.all(
      directPayments.map((payment) => PaymentValidationUtils.getDetailedDirectPaymentOption(payment, account))
    );
  ...
  static async findFeasibleContractPayments(contractPayments: PaymentOption[]): Promise<{
    ...
    const results = await Promise.all(
      contractPayments.map((payment) => PaymentValidationUtils.getDetailedContractPaymentOption(payment, account))
    );
