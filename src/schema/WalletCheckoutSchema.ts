...
export const PaymentOptionSchema = z
  .object({
    asset: z.string().min(1, 'Asset is required').refine(isValidCAIP19AssetId, 'Invalid CAIP-19 asset'),
  ...
  .refine(
    (data) => (data.recipient && !data.contractInteraction) || (!data.recipient && data.contractInteraction),
 ...
 */
export function isValidContractInteraction(contractInteraction: ContractInteraction | undefined): boolean {
